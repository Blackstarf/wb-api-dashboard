    ...
    <div class="chart-header">
      ...
      <div class="chart-controls">
        ...
        <select v-model="dataType" @change="updateChart">
          <option value="amount">Сумма продаж</option>
...
const prepareChartData = () => {
  ...
  props.data.forEach(item => {
    ...
    if (!groupedByDate[date]) {
      groupedByDate[date] = { 
        totalAmount: 0, 
        totalQuantity: 1, // Каждая запись - одна продажа
        totalDiscount: 0,
        count: 0
      };
    }
    ...
    datasets: [
      {
        ...
        backgroundColor: chartType.value === 'bar' ? 
          color + 'CC' : // Добавляем прозрачность для bar
          color + '33', // Добавляем прозрачность для line
          ...
          callbacks: {
            label: function(context) {
              ...
              if (context.parsed.y !== null) {
                ...
                } else if (dataType.value === 'quantity') {
...
const getChartTitle = () => {
  switch (dataType.value) {
    case 'amount': return 'Динамика суммы продаж по датам';
    case 'quantity': return 'Динамика количества продаж по датам';
    case 'discount': return 'Динамика средней скидки по датам';
    default: return 'Динамика продаж';
  }
};
...
const getYAxisLabel = () => {
  switch (dataType.value) {
    ...
    case 'quantity': return 'Количество (шт)';
